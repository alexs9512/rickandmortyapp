...
class CharacterListViewModel: ObservableObject {
    ...
    func fetchCharacters() {
        guard let url = URL(string: "https://rickandmortyapi.com/api/character") else { // hacerla explicita
        ...
        Task {
            do {
                ...
                let (data, _) = try await URLSession.shared.data(from: url) // variable para el tipo de data
                
                // 2. Decodificamos el JSON de forma segura
                let decoder = JSONDecoder() // hacer mas explicita 
